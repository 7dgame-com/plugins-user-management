...
async function checkInviteCode() {
  ...
  try {
    await api.get(`/invitations/${inviteCode.value}/check`)
    // Valid invitation
  } catch (err: any) {
    inviteError.value = err.response?.data?.error || t('register.messages.failed')
  } finally {
    checking.value = false
  }
}
...
async function sendCode() {
  ...
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('register.messages.failed'))
  } finally {
