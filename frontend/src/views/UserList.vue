...
async function fetchUsers() {
  ...
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('user.messages.fetchFailed'))
  } finally {
    loading.value = false
  }
}
...
async function handleRoleChange(row: any, newRole: string) {
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('user.messages.roleChangeFailed'))
  }
}
...
async function handleDelete(id: number) {
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('user.messages.deleteFailed'))
  }
}
