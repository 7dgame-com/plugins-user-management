...
async function fetchInvitations() {
  ...
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('invitation.messages.fetchFailed'))
  } finally {
    loading.value = false
  }
}
...
async function createInvitation() {
  ...
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('invitation.messages.generateFailed'))
  } finally {
...
async function revokeInvitation(code: string) {
  try {
    ...
    ElMessage.error(err.response?.data?.error || t('invitation.messages.revokeFailed'))
  }
}
...
async function viewRecords(row: Invitation) {
  ...
  try {
    const { data } = await api.get(`/invitations/${row.code}/records`)
    records.value = data
  } catch (err: any) {
    ElMessage.error(err.response?.data?.error || t('invitation.messages.recordsFailed'))
  } finally {
