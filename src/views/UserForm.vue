...
function assertPluginUserWriteSucceeded(response: { data?: unknown }): void {
  ...
  const error = Object.assign(new Error(message), {
    response: {
      status: 200,
      // Do not pass an unexpected success-shaped user payload to the generic
      // validation formatter: it would render username/role/nickname as an
      // error message and could disclose unrelated response fields.
      data: {
        code: body?.code,
        message,
      },
